			      ...
			      int *extrasize, unsigned char *extra)
{
    ...
    if (!filereadint32_t(file, &sig, "not a valid solution file"))
	return FALSE;
    if (sig != CSSIG)
	return fileerr(file, "not a valid solution file");
    if (!filereadint8_t(file, &n, "not a valid solution file"))
	return FALSE;
    if (n != ruleset)
	return fileerr(file, "solution file is for a different ruleset"
			     " than the level set file");
    if (!filereadint16_t(file, &f, "not a valid solution file"))
	return FALSE;
    *flags = (int)f;
...
{
    return filewriteint32_t(file, CSSIG, NULL)
	&& filewriteint8_t(file, ruleset, NULL)
	&& filewriteint16_t(file, flags, NULL)
	&& filewriteint8_t(file, extrasize, NULL)
...
static int writesolutionsetname(fileinfo *file, char const *setname)
{
    ...
    n = strlen(setname) + 1;
    return filewriteint32_t(file, n + 16, NULL)
...
static int readsolution(fileinfo *file, gamesetup *game)
{
    ...
    if (!filereadint32_t(file, &size, NULL) || size == 0xFFFFFFFF)
	return FALSE;
    if (!size)
	return TRUE;
    game->solutionsize = size;
...
static int writesolution(fileinfo *file, gamesetup const *game)
{
	...
	if (!filewriteint32_t(file, game->solutionsize, "write error")
			|| !filewrite(file, game->solutiondata,
				      game->solutionsize, "write error"))
	return FALSE;
    } else if (game->sgflags & SGF_HASPASSWD) {
	if (!filewriteint32_t(file, 6, "write error")
			|| !filewriteint16_t(file, game->number, "write error")
...
int savesolutionlevel(gameseries *series)
{
    ...
    f = fileskip(&series->savefile, 5, NULL)
	    && filewriteint16_t(&series->savefile, series->currentlevel, NULL);
    fileclose(&series->savefile, NULL);
    return f;
}
...
int loadsolutionsetname(char const *filename, char *buffer)
{
    ...
    if (!filereadint32_t(&file, &dwrd, NULL) || dwrd != CSSIG)
	goto badfile;
    if (!filereadint32_t(&file, &dwrd, NULL))
	goto badfile;
    dwrd = (dwrd >> 24) & 0xFF;
    if (!fileskip(&file, dwrd, NULL) || !filereadint32_t(&file, &dwrd, NULL))
	goto badfile;
    size = dwrd - 16;
    if (size <= 0)
	goto nosetname;

    if (!filereadint16_t(&file, &word, NULL)
				|| !filereadint32_t(&file, &dwrd, NULL))
	goto badfile;
    if (word || dwrd)
	goto nosetname;
